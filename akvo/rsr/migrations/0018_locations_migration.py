...
class Migration(DataMigration):
    ...
    def migrate_locations(self, orm, model):
        obj_type = ContentType.objects.get_for_model(model)
        # model.__name__ resolves to Project or Organisation
        location_model = getattr(orm, "%sLocation" % model.__name__)

        for obj in model.objects.all():
            found_primary = False # used to eliminate multiple primary locations
            locations = orm.Location.objects.filter(content_type__pk=obj_type.id, object_id=obj.id)
            for location in locations:
                ...
                new_location.save()
                if not found_primary and location.primary:
                    new_location.primary = found_primary = True
                    new_location.save()
                    obj.primary_location = new_location
                    obj.save()

    def forwards(self, orm):
        self.migrate_locations(orm, orm.Project)
        self.migrate_locations(orm, orm.Organisation)
