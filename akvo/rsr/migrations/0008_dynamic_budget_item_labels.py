...
class Migration(DataMigration):
    ...
    def backwards(self, orm):
        # backwards migration will only work with a dataset compatible with the old choices for BudgetItems.
        # tuple of the labels we can migrate backwards, if new ones are added we must bail
        OLD_ITEM_CHOICES = (u'employment', u'building', u'training', u'maintenance', u'management', u'other 1',)
        for item in orm.BudgetItem.objects.all():
            if not item.label.label in OLD_ITEM_CHOICES:
                raise RuntimeError("Cannot reverse this migration, budget items contain data that can't be coerced into the old DB structure.")
